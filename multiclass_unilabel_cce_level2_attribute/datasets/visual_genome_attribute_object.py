...
class VisualGenomeAttributeDataset():
...
class VisualGenomeAttributeCatalog():
    def __init__(self, split, image_dir, preprocessed_data_dir, preprocessed_data_file, dump_data_path, vocab_file, cluster_file,
        ...
        data_raw = {k:v for k,v in pkl.load(open(os.path.join(self.data_dir, preprocessed_data_file), 'rb'), encoding='latin1').items()}
        self.vocab = self.get_vocab(vocab_file, data_raw)
        self.clusters, self.clusters_inv = self.get_clusters(cluster_file)
        self.image_concepts_glove_emb = pkl.load(open(os.path.join(self.data_dir, image_concepts_glove_emb_file), 'rb'), encoding='latin1')
        self.attributes_glove_emb = pkl.load(open(os.path.join(self.data_dir, attributes_glove_emb_file),'rb'), encoding='latin1')
        glove_embedding_dim = self.attributes_glove_emb[list(self.attributes_glove_emb.keys())[0]].shape[0]
        ...
        for k,v in self.clusters_inv.items():
            glove_embs = np.asarray([self.attributes_glove_emb[vi] for vi in v])
    ...
    def get_datasets(self):
        ...
        for cluster_id in self.data:
            ...
            glove_embs = np.asarray([self.attributes_glove_emb[vi] for vi in self.clusters_inv[cluster_id]], dtype=np.float32)
            dataset = VisualGenomeAttributeDataset(self.image_dir, data, label, vocab, maxlen, glove_embs)
    ...
    def get_clusters(self, cluster_file):
        ...
        for k,v in json.load(open(os.path.join(self.data_dir, cluster_file))).items():
            ...
            for vi in v:
                if vi in self.vocab:
                    if vi in clusters:
                         print ('Isssue with ',vi)
                    clusters[vi] = cluster_id
        '''for k,v in json.load(open(os.path.join(self.data_dir, cluster_file))).items():
            if k in clusters or k not in data_raw:
                continue
            for vi in v:
                if vi in clusters:
                    clusters[k] = clusters[vi]
            if k in clusters:
                continue
            cluster_id += 1
            clusters[k] = cluster_id
            for vi in v:
                if vi not in clusters:
                    if vi not in data_raw:
                        continue
                    clusters[vi] = cluster_id
        ...
        for k in set(self.vocab.keys())-set(clusters.keys()):
            ...
            if k not in clusters:
                cluster_id += 1
                clusters[k] = cluster_id
                #print ('added a new cluster for ', k, 'searched for ', searched_for)
        '''
        print (max(clusters.values()))
        clusters_inv = {}        
