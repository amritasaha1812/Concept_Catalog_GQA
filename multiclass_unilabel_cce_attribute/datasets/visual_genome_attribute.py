...
class VisualGenomeAttributeDataset():
    def __init__(self, split, image_dir, preprocessed_data_dir, preprocessed_data_file, dump_data_path, vocab_file, cluster_file,
                 image_concepts_glove_emb_file, gpu_ids, cluster_classify, shuffle_data, sort_by):
        self.split = split
        self.sort_by = sort_by
        self.data_dir = preprocessed_data_dir
        self.dump_data_path = dump_data_path
        print ('self.dump_data_path ', self.dump_data_path)
        ...
        if preprocessed_data_file:
             data_raw = {k:v for k,v in pkl.load(open(os.path.join(self.data_dir, preprocessed_data_file), 'rb'), encoding='latin1').items() if k not in self.stopwords}
        else:
             data_raw = None
    ...
    def get_vocab(self, vocab_file, data_raw):
        ...
        for k,words in json.load(open(os.path.join(self.data_dir, vocab_file))).items():
            added = False
            for word in words:
                if word in data_raw and word not in vocab:
                    vocab[word] = id
                    added = True
            if added and len(words)>0:
                id += 1
        for k in set(data_raw)-set(vocab):
            print ('added ', k, 'in vocb')
            vocab[k]= id
            id += 1
    ...
    def get_clusters(self, cluster_file, data_raw):
        ...
        for k in set(data_raw.keys())-set(clusters.keys()):
            ...
            if k not in clusters:
                ...
                cluster_id += 1        
                print ('added a new cluster for ', k)
        clusters_inv = {}        
